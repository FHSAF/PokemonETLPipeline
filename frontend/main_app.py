	...
	for index, pokemon in filtered_df.iterrows():
		with cols[index % 2]:
			...
			with st.expander("View Insights"):
				# Display Stat Radar Chart
				st.plotly_chart(create_radar_chart(pokemon), use_container_width=True)
				
				# Display the Abilities List
				st.subheader("Abilities")
				# Check if the abilities list is not empty and the first item is not None
				if pokemon['abilities'] and pokemon['abilities'][0] is not None:
					for ability in pokemon['abilities']:
						# Format the ability name nicely (e.g., 'solar-power' -> 'Solar Power')
						st.markdown(f"- {ability.replace('-', ' ').title()}")
				else:
					st.caption("No abilities listed.")
